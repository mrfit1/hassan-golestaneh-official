...
## Contact form
The Contact page is four-language and sends submissions to `info@hassangolestaneh.com` through FormSubmit's AJAX endpoint. On the first real submission, FormSubmit may send a one-time activation email to that address. Because the address is routed through Cloudflare Email Routing, confirm that activation from the destination inbox once; later submissions should arrive normally.
